/* Board Sensor library for Arduino
 * 
 * Based in MPU6050_light made by: Romain JL. Fétick (github.com/rfetick)
...
BoardSensor::BoardSensor(TwoWire &w){
  wire = &w;
...
byte BoardSensor::begin(int gyro_config_num, int acc_config_num){
  // changed calling register sequence [https://github.com/rfetick/BoardSensor_light/issues/1] -> thanks to augustosc
  byte status = writeData(BoardSensor_PWR_MGMT_1_REGISTER, 0x01); // check only the first connection with status
  writeData(BoardSensor_SMPLRT_DIV_REGISTER, 0x00);
  writeData(BoardSensor_CONFIG_REGISTER, 0x00);
...
byte BoardSensor::writeData(byte reg, byte data){
...
byte BoardSensor::readData(byte reg) {
  wire->beginTransmission(address);
  wire->write(reg);
...
byte BoardSensor::setGyroConfig(int config_num){
  byte status;
  switch(config_num){
    case 0: // range = +- 250 deg/s
	  gyro_lsb_to_degsec = 131.0;
	  status = writeData(BoardSensor_GYRO_CONFIG_REGISTER, 0x00);
	  break;
	case 1: // range = +- 500 deg/s
	  gyro_lsb_to_degsec = 65.5;
	  status = writeData(BoardSensor_GYRO_CONFIG_REGISTER, 0x08);
	  break;
	case 2: // range = +- 1000 deg/s
	  gyro_lsb_to_degsec = 32.8;
	  status = writeData(BoardSensor_GYRO_CONFIG_REGISTER, 0x10);
	  break;
	case 3: // range = +- 2000 deg/s
	  gyro_lsb_to_degsec = 16.4;
	  status = writeData(BoardSensor_GYRO_CONFIG_REGISTER, 0x18);
	  break;
	default: // error
	  status = 1;
	  break;
...
byte BoardSensor::setAccConfig(int config_num){
  byte status;
  switch(config_num){
    case 0: // range = +- 2 g
	  acc_lsb_to_g = 16384.0;
	  status = writeData(BoardSensor_ACCEL_CONFIG_REGISTER, 0x00);
	  break;
	case 1: // range = +- 4 g
	  acc_lsb_to_g = 8192.0;
	  status = writeData(BoardSensor_ACCEL_CONFIG_REGISTER, 0x08);
	  break;
	case 2: // range = +- 8 g
	  acc_lsb_to_g = 4096.0;
	  status = writeData(BoardSensor_ACCEL_CONFIG_REGISTER, 0x10);
	  break;
	case 3: // range = +- 16 g
	  acc_lsb_to_g = 2048.0;
	  status = writeData(BoardSensor_ACCEL_CONFIG_REGISTER, 0x18);
	  break;
	default: // error
	  status = 1;
	  break;
...
void BoardSensor::setGyroOffsets(float x, float y, float z){
  gyroXoffset = x;
  gyroYoffset = y;
  gyroZoffset = z;
}

void BoardSensor::setAccOffsets(float x, float y, float z){
  accXoffset = x;
  accYoffset = y;
  accZoffset = z;
}

void BoardSensor::setFilterGyroCoef(float gyro_coeff){
...
void BoardSensor::setFilterAccCoef(float acc_coeff){
  setFilterGyroCoef(1.0-acc_coeff);
}

/* CALC OFFSET */

void BoardSensor::calcOffsets(bool is_calc_gyro, bool is_calc_acc){
...
void BoardSensor::fetchData(){
  wire->beginTransmission(address);
  wire->write(BoardSensor_ACCEL_OUT_REGISTER);
...
void BoardSensor::update(){
  ...
  // https://github.com/gabriel-milan/TinyBoardSensor/issues/6
...
int BoardSensor::Hall(){
     HallVal = hallRead();  
     return HallVal;    
}

int BoardSensor::Touch(int TouchPin){
    TouchVal = touchRead(TouchPin);   
    return TouchVal; 
}

int BoardSensor::Resistance(int ModPin,int R_board){
  ADC = analogRead(ModPin);
  V = ADC * (Vin/4095.0);
  R = R_board * (1/(Vin/V-1));
  return R;
}